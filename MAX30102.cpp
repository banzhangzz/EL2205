...
Timer tmr;
I2C max30102i2c(PB_14 , PB_10 ); //max30102 i2c
//DigitalIn MAX30102_INT(PA_8);

uint8_t max30102_Buffer[I2C_BUFFER_LENGTH]; //i2c buffer
//变量
MAX30105 sensor;
...
float MAX30105::readTemperature() {
  ...
  while (millis() - startTime < 100)
  {
    //uint8_t response = readRegister8( MAX30105_DIETEMPCONFIG); //Original way
...
uint16_t MAX30105::check(void)
{
  ...
  if (readPointer != writePointer)
  {
    ...
    while (bytesLeftToRead > 0)
    {
      ...
      memset(max30102_Buffer,0,I2C_BUFFER_LENGTH);
      Read_Len(MAX30105_FIFODATA,toGet,max30102_Buffer);
     ...
     // _i2cPort->requestFrom(MAX30105_ADDRESS, toGet);
      ...
      while (toGet > 0)
      {
        ...
        s.b[3] = 0;
        s.b[2] = max30102_Buffer[Getcnt++];
        s.b[1] = max30102_Buffer[Getcnt++];
        s.b[0] = max30102_Buffer[Getcnt++];
...
uint8_t MAX30105::readRegister8(uint8_t Register_Address)
{
    char cmd[3];
    cmd[0] = Register_Address;
    max30102i2c.write(max30102_WR_address,cmd,1);
    max30102i2c.read(max30102_WR_address,cmd,1);
    return  cmd[0]; 
}
...
void MAX30105::writeRegister8( uint8_t reg, uint8_t value) 
{
    ...
    cmd[0] = reg;
    cmd[1] = value;
	max30102i2c.write(max30102_WR_address,cmd,2);      
}
uint32_t MAX30105::millis(void)
{
    return duration_cast<milliseconds>(tmr.elapsed_time()).count();
}
/**
 * @brief max30102 I2C连续读
 * @param reg:寄存器地址
 * @param len:长度
 * @param buf:缓冲区地址
 * @return 状态 0成功 其他失败
 */
uint8_t MAX30105::Read_Len(uint8_t reg, uint8_t len, uint8_t *buf)
{
    char cmd[10];
    cmd[0] = reg;
    max30102i2c.write(max30102_WR_address,cmd,1);
//	HAL_Delay(1);
    return max30102i2c.read(max30102_WR_address,(char*)buf,len);
}

void max30102_init(void)
{
  max30102i2c.frequency(I2C_SPEED_FAST);
  tmr.start();
...
int getHeartrate(int *btavg,float *btPm)
{
  ...
  {
        //We sensed a beat!
        long delta = sensor.millis() - lastBeat;
        lastBeat = sensor.millis();
